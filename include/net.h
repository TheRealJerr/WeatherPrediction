...
#define SOCKINTOSOCK(PTR) (struct sockaddr*)(PTR) 
    ...
    public:
        ...
        {
            user_count++;
            // _socket = createFromPosixSocket(_sock_communicate);
        }
        ...
        // void readBuffer(Buffer*); // 这是version2提供的通过vector存储数据

        // void sendBuffer(Buffer&); // 这是version2提供的通过vector存储数据
        ...
        std::optional<boost::asio::ip::tcp::socket> createFromPosixSocket(int);
